...
function fitRightJoin(content, parent, $from, $to, depth, openStart, openEnd) {
   ...
   if (openStart < 0) match = parentNode.contentMatchAt(matchCount);
         ...
         if (inner) {
            ...
            if (joinable.size)
               return content
                  ...
                  .append(joinable)
                  .addToEnd(last);
            else return content.replaceChild(count - 1, last);
         }
      }
   }
   if (openEnd > 0)
...
function fitRightClosed(node, openEnd, $from, depth, openStart) {
   ...
   else match = node.contentMatchAt(count);
...
class Frontier {
   ...
   placeSlice(fragment, openStart, openEnd, pass, parent) {
      ...
      if (pass > 2 && result.size && openStart == 0) {
         let child = result.content.firstChild,
